...
// Global functions & variables 
	...
	// Reset email form fields
		...
		var resetEmail = function () {$('.email-address').focus(function(){$('.alert').css('display', 'none');$('#large-btn').attr('value', 'REGISTER').removeAttr("disabled").css('background', '#50aaf0').css('cursor', 'pointer');$(this).css('border', '1px solid #e1e8ed');$(".password").css('border', '1px solid #e1e8ed');});}
	...
	// Form alerts handling
		...
		// Sub Error email sent
		var alertEmailSent = function(){$('.alert').empty().removeClass('success').append('You need to verify your account. Please check your email').css('display', 'block');}
		// Success email reset
	...
	$('.process-login').submit(function(){
       ...
       $.getJSON('https://v43.amplefi.co/auth?callback=?', {
			...
			if (data.status == 'authorized'){
				...
				window.location = '../login/?previousRegister=true'	
			}
			// Error on form 
			...
			console.log("email autorized email sent");
			window.location = '../login/?showRegister=true'		
			}	
			else {
