...
// Global functions & variables 
    ...
    if (password != confirmPassword){
     ...
     $('.password-confirm').css('border', '1px solid #a3c5a0');
		...
		var resetPassword = function () {$('.password').focus(function(){$('.alert').css('display', 'none');$('#large-btn').attr('value', 'Set password').removeAttr("disabled").css('background', '#50aaf0').css('cursor', 'pointer');$(this).css('border', '1px solid #e1e8ed');});}
        ...
        $.getJSON('https://v43.amplefi.co/auth_set_pwd?callback=?', {
            ...
            function (data) {
				if (data.status == 'ok'){
					...
					$('#large-btn').attr('value','Set password');
